...
/**
 * <b> This program calls SicBoV4 </b> <br>
 ...
 * Ask user type "A" for play again <br>
 * Show the result is what game you play and win or loose in that round <br>
 * 
...
public class SicBoV4 {
    ...
    // gamePlay keeps track of what the player has played in the game
    static String[] gamePlay = new String[MAX_INPUT];
    // gameResults keeps track of what the result of the game
     ...
     * Set game summary 1 round <br>
     * 
     * @param playGameNum is get value from scanner input in method yourChoice
     */
    public static void playGame(double playGameNum){ 
        if (playGameNum == 1) { // if get number from playGame is 1
            gameOne();
            currentInput += 1; // add current
        }
     ...
     * Get the result to game summary <br>
     * 
     */
        ...
        while (gameOneBoo) {
            ...
            if(hl_input.equalsIgnoreCase("h")){ // If input is h and run below the program 
                ...
                if(total>0 && total <=9){ // If the total is over 0 but less than or equal to 9
                    ...
                    gameResult[currentInput] = "You loose 10 Baht."; // set gameResult
                }
                else{ // More than 9 is equal high you win
                    System.out.println("You win 20 Bath");
                    gameResult[currentInput] = "You loose 20 Baht."; // set gameResult
                }   
            }
            ...
            gamePlay[currentInput] = hl_input; // set what user input in the program to gamePlay[]
            gameOneBoo = false;
        }
    }
    ...
    public static void gameTwo(){
        boolean gameTwoBoo = true; //set Boolean for use in while loop
        ...
        String numGameTwo = inputUser.next(); // Read user input
        while (gameTwoBoo) {
            try { // try to run program below
                Integer num_check = Integer.parseInt(numGameTwo); // Change string to integer
                if(num_check < 7 && num_check > 0 ){ // if input is 1 to 6
                    ...
                    if(totalMoney == 0){ // If user input not equal to all of the dice
                        ...
                        gameResult[currentInput] = "You win "+ totalMoney +" Bath."; // set gameResult
                    }
                    gamePlay[currentInput] = numGameTwo; // set what user input in the program to gamePlay[]
                }
    ...
    /**
     * <b> This is gameTotal method </b> <br>
     * Will show the game summary <br>
     */
    public static void gameTotal() {
        for (int j = 0; j < currentInput; j++) {
            System.out.println("Game " + j + " :");
            System.out.println("You have bet on " + gamePlay[j]);
            System.out.println(gameResult[j]);
        }
    }
}
